...
void loop() {
  // Read sensor values and update statuses for slots 1 to 8
  for (int i = 0; i < 8; i++) {
  ...
  // Display status on the LCD for slots 1 to 6
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("S1:");
  ...
  // Control gate based on slot status
  if (allFull && sensorValues[6] == HIGH && sensorValues[7] == HIGH) {  //CHANGE THE  VALUE TO LOW
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("All Slots Full");
    servo1.write(0); // Keep the gate closed
  } else if (!allFull && (sensorValues[6] == LOW || sensorValues[7] == LOW)) {
    // Open the gate if slots are available and either sensor 7 or 8 is triggered
    servo1.write(90); // Open gate
    delay(3000);
    servo1.write(0);  // Close gate after delay
  } else {
    servo1.write(0); // Keep gate closed if conditions are not met
  }
